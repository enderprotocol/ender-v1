...
describe("Ender Bond deposit and withdraw", async() => {
    ...
    describe("Ender Staking contract test", async() => {
        ...
        it("setAddress is reverted with ZeroAddress custom error", async () => {
            await expect(enderStaking.setAddress(ethers.ZeroAddress, 1)).to.be.revertedWithCustomError(enderStaking, "ZeroAddress");
        });

        it("setAddress is reverted with invalid caller", async () => {
            await expect(enderStaking.connect(signer1).setAddress(stEthAddress, 1)).to.be.revertedWith("Ownable: caller is not the owner");
        });

        it("stake is reverted with InvalidAmount custom error", async () => {
            await expect(enderStaking.stake(0, [signer1.address, "0", signature1])).to.be.revertedWithCustomError(enderStaking, "InvalidAmount");
        }); 
        ...
        it("stake is called with isWhitelisted", async () => {
            ...
            const signerAddress = await mockEnderStaking.signer();
            ...
            await expect(mockEnderStaking.connect(signer1).stake(stakingAmount, userSign)).to.be.not.revertedWithCustomError(mockEnderStaking, "NotWhitelisted");
        ...
        it("stake is reverted with NotWhitelisted owing to wrong signer", async () => {
            ...
            await expect(mockEnderStaking.connect(signer1).stake(stakingAmount, userSign)).to.be.revertedWithCustomError(mockEnderStaking, "NotWhitelisted");
        ...
        it("stake is reverted with NotWhitelisted owing to wrong sender", async () => {
            ...
            await expect(mockEnderStaking.connect(signer2).stake(stakingAmount, userSign)).to.be.revertedWithCustomError(mockEnderStaking, "NotWhitelisted");
        ...
        it("stake is reverted with not allowed staking enable", async () => {
            ...
            await expect(enderStaking.stake(10, [signer1.address, "0", signature1])).to.be.revertedWithCustomError(enderStaking, "NotAllowed");
        ...
        it("unstake is reverted with InvalidAmount custom error owing to balance isn't enough", async () => {
            ...
            await expect(enderStaking.connect(signer1).unstake(sEndTokenBalance + BigInt(1))).to.be.revertedWithCustomError(enderStaking, "InvalidAmount");
        });
        ...
        it("epochStakingReward is reverted with NotAllowed custom error owing to asset isn't stEth", async () => {
            await expect(enderStaking.epochStakingReward(endTokenAddress)).to.be.revertedWithCustomError(enderStaking, "NotAllowed");
        });

        it("_hash function testing", async () => {
            const userSign = [signer1.address, "0", signature1];

            const hashValue = await mockEnderStaking.hash(userSign);

            const typeHash = ethers.keccak256(ethers.toUtf8Bytes("userSign(address user,string key)"));

...
async function main() {
    ...
    fs.writeFileSync(
        path.join(dirPath, "EndToken.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    const enderBondInstance = await upgrades.deployProxy(
        EnderBond,
        [endTokenAddress, enderSinature],
        {
            initializer: "initialize",
        },
    );
    ...
    fs.writeFileSync(
        path.join(dirPath, "EnderBond.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    fs.writeFileSync(
        path.join(dirPath, "BondNFT.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    const enderTreasuryInstance = await upgrades.deployProxy(
        EnderTreasury,
    ...
    const enderTreasuryImplAddress =
        await upgrades.erc1967.getImplementationAddress(enderTreasuryAddress);
    fs.writeFileSync(
        path.join(dirPath, "EnderTreasury.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    const enderStakingImplAddress =
        await upgrades.erc1967.getImplementationAddress(enderStakingAddress);
    fs.writeFileSync(
        path.join(dirPath, "EnderStaking.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    const enderLidoStrategyInstance = await upgrades.deployProxy(
        EnderLidoStrategy,
        [enderTreasuryAddress, LidoAgentAddress],
        {
            initializer: "initialize",
        },
    );
    ...
    const enderLidoStrategyImplAddress =
        await upgrades.erc1967.getImplementationAddress(enderLidoStrategyAddress);
    fs.writeFileSync(
        path.join(dirPath, "EnderLidoStrategy.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    const enderELStrategyInstance = await upgrades.deployProxy(
        EnderELStrategy,
        [enderTreasuryAddress, elStrategyManager],
        {
            initializer: "initialize",
        },
    );
    ...
    const enderELStrategyImplAddress =
        await upgrades.erc1967.getImplementationAddress(enderELStrategyAddress);
    fs.writeFileSync(
        path.join(dirPath, "EnderELStrategy.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    await enderTreasuryInstance.setStrategy(
        [enderLidoStrategyAddress, enderELStrategyAddress],
        true,
    );
    ...
    const enderOracleImplAddress =
        await upgrades.erc1967.getImplementationAddress(enderOracleAddress);
    fs.writeFileSync(
        path.join(dirPath, "EnderOracle.json"),
        JSON.stringify(
            {
                goerli: {
    ...
    // set ETH price feed
    await enderOracleInstance.setFeeds(
        [ethers.ZeroAddress, stETHTokenAddr],
        [goerliETHFeed, goerliETHFeed],
    );
